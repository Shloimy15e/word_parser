...
class DosReader(InputReader):
    ...
    def _is_dos_encoded(cls, file_path: Path) -> bool:
        ...
        try:
            ...
            try:
                ...
                total_chars = len(
                    [c for c in text if c.isprintable() and not c.isspace()]
                )
    ...
    def read(self, file_path: Path) -> Document:
        ...
        # Check if this is a mekoros/mkoros file (skip paragraph reconstruction)
        filename_stem = file_path.stem if file_path.suffix else file_path.name
        is_mekoros = bool(re.match(r"^me?koros0*(\d*)$", filename_stem, re.IGNORECASE))

        # Check for paragraph marker 0x14 (common in some DOS files)
        if "\x14" in text:
            # Split by paragraph marker
            raw_paragraphs = text.split("\x14")
            for raw_para in raw_paragraphs:
                # Clean DOS formatting codes and garbage
                cleaned_text = clean_dos_text(raw_para)
                # Sanitize text to remove invalid XML characters
                cleaned_text = sanitize_xml_text(cleaned_text)

                cleaned_text = cleaned_text.strip()
                if cleaned_text:
                    # Replace newlines with spaces to make it a continuous paragraph
                    # BUT skip this reconstruction for mekoros/mkoros files
                    if not is_mekoros:
